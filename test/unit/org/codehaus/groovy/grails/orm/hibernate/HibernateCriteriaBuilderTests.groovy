...
class HibernateCriteriaBuilderTests extends AbstractGrailsHibernateTests {
	...
	void testSizeErrorMessages() {
		...
		methodNames.each { methodName ->
			...
			assert "Call to [${methodName}] with propertyName [somePropertyName] and size [0] not allowed here." == errorMessage
		}
	}
	...
	void testUniqueResult() {
		...
		GrailsDomainClass domainClass = grailsApplication.getArtefact(DomainClassArtefactHandler.TYPE, clazzName)
